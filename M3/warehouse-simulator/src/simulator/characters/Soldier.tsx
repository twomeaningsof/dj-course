...
export const Soldier = forwardRef<SoldierRef, SoldierProps>(({ initialPosition }, ref) => {
  ...
  const PATROL_SPEED = 0.5;      // units per second when patrolling
  const CHASE_SPEED = 2.0;       // units per second when chasing
  const AGGRO_DISTANCE = 10;     // start chasing at this distance
  const ATTACK_DISTANCE = 3;     // stop and attack at this distance
  // AI State
  enum SoldierState {
    IDLE = 'idle',
    PATROL = 'patrol',
  ...
  const [aiState, setAiState] = useState<SoldierState>(SoldierState.IDLE);
  ...
  useFrame((state, delta) => {
    ...
    if (!isDead && camera) {
      ...
      const distanceToPlayer = Math.sqrt(
        Math.pow(playerPos.x - position[0], 2) +
        Math.pow(playerPos.z - position[2], 2)
      );

      // AI State Transitions
      if (distanceToPlayer < ATTACK_DISTANCE) {
        if (aiState !== SoldierState.ATTACK) {
          setAiState(SoldierState.ATTACK);
          console.log('Soldier entering ATTACK state');
        }
      } else if (distanceToPlayer < AGGRO_DISTANCE) {
        if (aiState !== SoldierState.CHASE) {
          setAiState(SoldierState.CHASE);
          console.log('Soldier entering CHASE state');
        }
      } else {
        if (aiState !== SoldierState.IDLE) {
          setAiState(SoldierState.IDLE);
          console.log('Soldier entering IDLE state');
        }
      }

      // Movement Logic
      if (aiState === SoldierState.CHASE) {
        const dirX = playerPos.x - position[0];
        const dirZ = playerPos.z - position[2];
        const length = Math.sqrt(dirX * dirX + dirZ * dirZ);

        if (length > ATTACK_DISTANCE) { // Don't overshoot
          // Normalize direction and move
          const normalizedX = dirX / length;
          const normalizedZ = dirZ / length;

          const newX = position[0] + normalizedX * CHASE_SPEED * delta;
          const newZ = position[2] + normalizedZ * CHASE_SPEED * delta;

          setPosition([newX, position[1], newZ]);
        }
      }

      // Log distance for testing (remove later)
      if (Math.random() < 0.01) { // Log occasionally to avoid spam
        console.log(`Soldier distance to player: ${distanceToPlayer.toFixed(2)}`);
      }
    }
